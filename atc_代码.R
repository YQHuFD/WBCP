setwd("E:\\药物联用项目\\操作\\相似性计算\\ATC")
library(data.table)
library(dplyr)
library(lsa)

# Read drugbank_id -> mesh_dui file
db2atc <- read.table("ATC.txt",sep="\t",header=T,check.names=F)
...
# Compute IDF
tf <- sim
idf <- log((nrow(sim)+1)/(colSums(sim)+1))
tfidf <- sim
...
# Write table
write.table(cos, "atc_similarity.txt", sep="\t", quote=F, row.names=T)
